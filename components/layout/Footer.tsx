...
function FooterHeading({ children }: { children: React.ReactNode }) {
  return (
    <div>
      ...
      <span className="mt-3 block h-px w-8 bg-gold-500/50" aria-hidden />
    </div>
  );
}
...
function PortalColumn({
  ...
  return (
    <motion.div variants={fadeUp}>
      ...
      <Badge tone="navy" className="mt-5 border-gold-500/30 text-gold-400">
        Coming Soon
      </Badge>
    </motion.div>
  );
...
function NewsletterForm() {
  ...
  if (state === "success") {
    return (
      <div className="mt-5 flex items-start gap-2.5 rounded-[var(--radius-md)] border border-gold-500/30 bg-white/5 p-4 text-sm text-white/70">
        <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-gold-400" strokeWidth={1.5} />
    ...
    <form onSubmit={handleSubmit} noValidate className="mt-5">
      ...
      <div className="flex gap-2">
        <input
          ...
          className="focus-ring w-full min-w-0 rounded-[var(--radius-md)] border border-white/15 bg-white/5 px-4 py-2.5 text-sm text-white placeholder:text-white/40 disabled:opacity-60"
        />
        <button
          type="submit"
          disabled={state === "loading"}
          aria-label="Subscribe"
          className="focus-ring flex shrink-0 items-center justify-center rounded-[var(--radius-md)] bg-gold-500 px-4 text-navy-950 transition-colors duration-200 ease-out hover:bg-gold-400 disabled:opacity-70"
        >
          {state === "loading" ? (
      ...
      <Container className="py-12 sm:py-14 lg:py-16">
        ...
        >
          ...
          <motion.div variants={fadeUp} className="relative col-span-2 max-w-sm lg:col-span-1 lg:pr-4">
            ...
            {/* Subtle campus line-art decoration */}
            <svg
              aria-hidden
              viewBox="0 0 320 120"
              ...
              <div className="mt-6 flex gap-3">
                {socials.map(({ key, href, label }) => (
                  <a
                    ...
                    rel="noreferrer"
                    aria-label={label}
                    className="focus-ring flex h-10 w-10 items-center justify-center rounded-full border border-white/15 text-white/70 transition-colors hover:border-gold-400 hover:text-gold-400"
                  >
